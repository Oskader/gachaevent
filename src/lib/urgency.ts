...
function srFormat(remainingMs: number): string {
  if (remainingMs <= 0) return 'Terminado'
  const days = Math.floor(remainingMs / DAY)
  const hours = Math.floor((remainingMs % DAY) / HOUR)
  if (days > 0) return `Quedan ${days} ${days === 1 ? 'día' : 'días'} y ${hours} h`
  if (hours > 0) return `Quedan ${hours} h`
  return `Quedan menos de una hora`
}
 ...
 */
export function countdownAt(endDate: string, now: number): Countdown {
